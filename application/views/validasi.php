	...
	<table id="tbl_data" class="table table-bordered table-striped" cellspacing="0" width="100%">
		...
		<tbody>
			...
			<tr>
				...
				<?php
				    if($this->session->userdata('prioritas') == 1) :
				    	if($dt->approval == 1 && $dt->validasi < 1 && $dt->validasi == 0) :
				?>
					 <a href="<?php echo base_url('validasi/approve?id='.$dt->no_pengajuan); ?>" title="Approve"><i class="fa fa-thumbs-o-up">&nbsp;</i></a> &nbsp; 
					 <a href="<?php echo base_url('validasi/reject?id='.$dt->no_pengajuan); ?>" title="Reject"><i class="fa fa-thumbs-o-down">&nbsp;</i></a>
				<?php endif; endif; ?>
				</td>
			</tr>
			<?php $no++; endforeach; ?>
